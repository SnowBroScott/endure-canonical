...
DATE: April 22, 2026
WHY: A mechanical power framework — gear stats, affixes, power-gated escalation — contradicts what Endure fundamentally is. If the power lives on a stat sheet, the game is lying about where effort matters. The simpler, more honest loop removes that contradiction. Aesthetic loot still carries meaning and reward without distorting the progression model. Content escalation through real-world effort keeps the game true to its premise.
REPLACES: The mechanical power framework described in the Core Gameplay Loop document — gear affixes, stat changes, and power-gated content escalation — is superseded in full.
STATUS: DRAFT

DECISION: Hybrids are a blend of two existing paths, not a new path. Sustained behavioral overlap across two class paths produces a hybrid identity. Six hybrids exist at launch — Björn (Strider + Forgeborn), Vargr (Strider + Unbroken), Haukr (Strider + Mystic), Naut (Forgeborn + Unbroken), Hjörtr (Forgeborn + Mystic), Naðr (Unbroken + Mystic). Names are Old Norse, universally legible animals, requiring no explanation. Three-way hybrids are deferred pending post-launch behavioral data.
Recognition operates via a cumulative weighted scale across all four paths. Primary path confirms at 100% cumulative weight. Hybrid emergence begins at approximately 25–30% split, progresses at ~32%, and confirms at ~40%. The same scale governs visual expression within the hybrid — weight distribution between the two paths determines how much of each is visually present on the character.
Hybrids carry a stable earned name with a primary and secondary path. Visual expression shifts dynamically to reflect current dominant path within the blend. Hybrid status is not permanent — sustained neglect of one path drifts over time. Earning and drift follow ceremonial narrative sequences of equal weight and tone. All ten identities (four base classes, six hybrids) are peers — equal in mythic weight, equal in cool factor.
DATE: April 21, 2026
WHY: Hybrids must emerge from real behavior, not be chosen. A recognition-based system grounded in cumulative effort preserves the integrity of the identity layer — you are what you do. Old Norse animal names are universally understood, carry mythic weight, and require no in-world justification. Equality between all ten identities prevents hybrid from reading as an upgrade path, which would distort player behavior and undermine the base classes. Three-way hybrids are deferred because the behavioral data to design them responsibly doesn't exist yet.
REPLACES: Nothing — new decision
STATUS: DRAFT
